...
import { findModelProfile, getCurrentModelProfile } from "../../core/modelRegistry.js";
...
export const ModelShowScreen: React.FC<ModelShowScreenProps> = ({ name }) => {
  ...
  if (!profile) {
    return (
      <Box flexDirection="column">
        ...
        <Text>`llmine model list` で登録済みのプロファイルを確認してください。</Text>
      </Box>
    );
  }
  ...
  return (
    <Box flexDirection="column" gap={1}>
      ...
      <Text>
        プロバイダ: {PROVIDER_LABELS[profile.provider] ?? profile.provider} ({profile.provider})
      </Text>
      <Text>モデルID: {profile.modelId}</Text>
      <Text>
        temperature: {profile.temperature !== undefined ? profile.temperature : "未設定"}
      </Text>
